...
public abstract class Move {
	...
	public static final class EnPassantMove extends PawnAttackMove {
		...
		public EnPassantMove(final Board board, final Piece movedPiece, final int destinationCoordinate,
				final Piece attackedPiece) {
			super(board, movedPiece, destinationCoordinate, attackedPiece);
		}
		...
		public Board execute() {
			...
			for (final Piece piece : this.board.currentPlayer().getActivePieces()) {
				if (!this.movedPiece.equals(piece)) {
					builder.setPiece(piece); // sets all pieces that aren't the jumping pawn in the same position
				}
			}
			...
			builder.setPiece(movedPawn); // adds moved pawn to the board
			builder.setEnPassantPawn(movedPawn); // sets moved pawn to enPassantPawn for new board
			builder.setMoveMaker(this.board.currentPlayer().getOpponent().getAlliance());
			return builder.build();
		}
		...
		public Board execute() {
			...
			for (final Piece piece : this.board.currentPlayer().getActivePieces()) {
				if (!this.movedPiece.equals(piece) && !this.castleRook.equals(piece)) {
					builder.setPiece(piece); // sets all pieces that aren't the king or rook in their old spots
				}
			}
			for (final Piece piece : this.board.currentPlayer().getOpponent().getActivePieces()) {
				builder.setPiece(piece);
